...
if [ "$1" = "unitd" -o "$1" = "unitd-debug" ]; then
    if find "/var/lib/unit/" -mindepth 1 -print -quit 2>/dev/null | grep -q .; then
        ...
        if find "/docker-entrypoint.d/" -mindepth 1 -print -quit 2>/dev/null | grep -q .; then
            ...
            while [ ! -S /var/run/control.unit.sock ]; do echo "$0: Waiting for control socket to be created..."; sleep 0.1; done
            ...
            while [ -S /var/run/control.unit.sock ]; do echo "$0: Waiting for control socket to be removed..."; sleep 0.1; done
